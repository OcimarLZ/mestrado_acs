...
def gerar_dados_evolucao_org_academica():
    """
    Conecta ao banco, consulta os dados de evolução de IES por organização acadêmica
    ...
    try:
        ...
        # Redefine models
        class Tp_organizacao_academica(Base):
        ...
        class Curso_censo(Base):
            ...
            org_academica = Column(Integer, ForeignKey('tp_organizacao_academica.codigo'))
            ies = Column(Integer)

        # Query with filters - now using Curso_censo
        results = session.query(
            Curso_censo.ano_censo,
            Tp_organizacao_academica.nome,
            sqlalchemy.func.count(Curso_censo.ies.distinct())
        ).join(
            Tp_organizacao_academica, Curso_censo.org_academica == Tp_organizacao_academica.codigo
        ).filter(
        ...
        df = pd.DataFrame(results, columns=['ano', 'organizacao', 'total'])
        pivot_df = df.pivot(index='ano', columns='organizacao', values='total').fillna(0)
        ...
        json_path = os.path.abspath('D:/ProjetosPY/inep/acs/dados_evolucao_org_academica.json')
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(final_json_data, f, ensure_ascii=False, indent=4)
        
        print(f"Dados de evolução por organização acadêmica salvos com sucesso em '{json_path}'")
...
if __name__ == '__main__':
    gerar_dados_evolucao_org_academica()
document.addEventListener('DOMContentLoaded', function() {
     fetch('educacao_superior_por_categoria_trancados.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('educacaoSuperiorPorCategoriaTrancadosChart').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Trancamentos por Categoria Administrativa (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
                    ...
                    scales: {
                        ...
                        y: {
                            title: {
                                display: true,
                                text: 'Quantidade de Trancamentos'
                            },
                            beginAtZero: true,
                            stacked: false
                        }
            ...
            document.getElementById('exportEducacaoSuperiorPorCategoriaTrancadosBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Trancamentos por Categoria");
                XLSX.writeFile(wb, "educacao_superior_por_categoria_trancados.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico de trancamentos por categoria administrativa:', error));
});

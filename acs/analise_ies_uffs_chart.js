document.addEventListener('DOMContentLoaded', function() {
     fetch('analise_ies_uffs.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('analiseIesUffsChart').getContext('2d');
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Dinâmica da UFFS (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
            ...
            document.getElementById('exportAnaliseIesUffsBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "UFFS");
                XLSX.writeFile(wb, "analise_ies_uffs.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico da UFFS:', error));
});

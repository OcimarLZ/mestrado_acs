document.addEventListener('DOMContentLoaded', function() {
     fetch('educacao_superior_por_categoria_matriculas.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('educacaoSuperiorPorCategoriaMatriculasChart').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Matrículas por Categoria Administrativa (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
            ...
            document.getElementById('exportEducacaoSuperiorPorCategoriaMatriculasBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Matriculas por Categoria");
                XLSX.writeFile(wb, "educacao_superior_por_categoria_matriculas.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico de matrículas por categoria administrativa:', error));
});

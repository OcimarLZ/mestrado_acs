...
SELECT 
     cc.ano_censo AS ano, 
     SUM(cc.qt_mat_branca) AS branca, 
     SUM(cc.qt_mat_parda) AS parda, 
     SUM(cc.qt_mat_preta) AS preta, 
     SUM(cc.qt_mat_indigena) AS indigena, 
     SUM(cc.qt_mat_amarela) AS amarela, 
     SUM(cc.qt_mat_cornd) AS naodeclarada 
 FROM 
     curso_censo cc 
 WHERE 
...
output_path = os.path.join(os.path.dirname(__file__), 'dados_discentes_matriculados_raca.json')
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=4)

print(f"JSON com dados de matriculados por raça gerado com sucesso em {output_path}")
...
# Query SQL para dados combinados por Modalidade de Ensino (A Distância - EAD)
SQL_QUERY_EAD_COMBINED = """
SELECT 
    cc.ano_censo, 
...
WHERE 
    ...
    AND tm.codigo = 2 -- Código para A Distância (EAD)
GROUP BY 
    cc.ano_censo, 
    tm.nome
ORDER BY 
...
df_ead_combined = pd.read_sql(SQL_QUERY_EAD_COMBINED, engine)

# Caminho para salvar o JSON
output_json_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'static', 'dados'))
os.makedirs(output_json_dir, exist_ok=True)
output_json_path_ead_combined = os.path.join(output_json_dir, 'educacao_superior_por_modalidade_ead_combined.json')

with open(output_json_path_ead_combined, 'w', encoding='utf-8') as f:
    df_ead_combined.to_json(f, orient='records', indent=4)

print(f"Dados combinados EAD por modalidade gerados em: {output_json_path_ead_combined}")
...
output_excel_path_ead_combined = os.path.join(output_excel_dir, 'educacao_superior_por_modalidade_ead_combined.xlsx')
df_ead_combined.to_excel(output_excel_path_ead_combined, index=False)

print(f"Excel de dados combinados EAD por modalidade gerado em: {output_excel_path_ead_combined}")

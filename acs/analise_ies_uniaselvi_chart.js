document.addEventListener('DOMContentLoaded', function() {
     fetch('analise_ies_uniaselvi.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('analiseIesUniaselviChart').getContext('2d');
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Dinâmica da Uniaselvi (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
            ...
            document.getElementById('exportAnaliseIesUniaselviBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Uniaselvi");
                XLSX.writeFile(wb, "analise_ies_uniaselvi.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico da Uniaselvi:', error));
});

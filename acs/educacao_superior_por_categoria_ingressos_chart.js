document.addEventListener('DOMContentLoaded', function() {
     fetch('educacao_superior_por_categoria_ingressos.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('educacaoSuperiorPorCategoriaIngressosChart').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Ingressos por Categoria Administrativa (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
            ...
            document.getElementById('exportEducacaoSuperiorPorCategoriaIngressosBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Ingressos por Categoria");
                XLSX.writeFile(wb, "educacao_superior_por_categoria_ingressos.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico de ingressos por categoria administrativa:', error));
});

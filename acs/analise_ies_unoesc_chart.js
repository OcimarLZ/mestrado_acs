document.addEventListener('DOMContentLoaded', function() {
     fetch('analise_ies_unoesc.json') 
        ...
        .then(data => {
            ...
            const ctx = document.getElementById('analiseIesUnoescChart').getContext('2d');
            new Chart(ctx, {
                type: 'line',
                data: {
                    labels: anos,
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Dinâmica da Unoesc (2014-2024)'
                        },
                        tooltip: {
                            mode: 'index',
                            intersect: false,
            ...
            document.getElementById('exportAnaliseIesUnoescBtn').addEventListener('click', function() {
                const ws = XLSX.utils.json_to_sheet(data);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Unoesc");
                XLSX.writeFile(wb, "analise_ies_unoesc.xlsx");
            });

        })
        .catch(error => console.error('Erro ao carregar dados do gráfico da Unoesc:', error));
});

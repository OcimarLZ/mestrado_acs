...
# Query SQL para Unochapecó (código 3151)
SQL_QUERY_UNOCHAPECO = """
SELECT 
    cc.ano_censo, 
...
df_unochapeco = pd.read_sql(SQL_QUERY_UNOCHAPECO, engine)

# Caminho para salvar o JSON
output_json_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'static', 'dados'))
os.makedirs(output_json_dir, exist_ok=True)
output_json_path_unochapeco = os.path.join(output_json_dir, 'analise_ies_unochapeco.json')

with open(output_json_path_unochapeco, 'w', encoding='utf-8') as f:
    df_unochapeco.to_json(f, orient='records', indent=4)

print(f"Dados da Unochapecó gerados em: {output_json_path_unochapeco}")
...
output_excel_path_unochapeco = os.path.join(output_excel_dir, 'analise_ies_unochapeco.xlsx')
df_unochapeco.to_excel(output_excel_path_unochapeco, index=False)

print(f"Excel da Unochapecó gerado em: {output_excel_path_unochapeco}")

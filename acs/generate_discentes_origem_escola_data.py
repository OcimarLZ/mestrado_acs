...
data = {
    'anos': df['ano'].tolist(),
    'publica': df['publica'].tolist(),
    'privada': df['privada'].tolist()
}

# Caminho para salvar o JSON
output_json_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'dados_discentes_origem_escola.json'))
